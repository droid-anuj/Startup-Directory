    ...
    <div className="mt-8 flex flex-wrap bg-black">
      ...
      {data.length > 0 ? (
        ...
        data.map((startup) => (
          <div
            key={startup._id}
            className={`flex-none w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/4 p-4`}
          >
            <div
              className="border border-white-300 p-4 mb-4 rounded h-full bg-white cursor-pointer transition-transform transform hover:scale-105"
              onClick={() => handleClick(startup)}
            >
              <h2 className="text-xl text-black font-bold mb-4 ml-4">
                {startup.StartupName}
              </h2>
              <hr />
      ...
      {selectedStartup && (
        <Startupdetails startup={selectedStartup} onClose={handleClose} />
      )}
    </div>
  );
}
